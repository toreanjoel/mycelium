defmodule Mycelium.Servers.Subserver do
  ...
  def init(args) do
    ...
    config = %Mycelium.Servers.Subserver.Config{
      pid: self(),
    ...
    base_state = %{
      config: config,
      channel_states: %{
        "lobby" => %Mycelium.Servers.Subserver.Structs{type: :default, state: %{}}
      }
    }
  ...
  def handle_call({:create_channel, channel_name, type, init_payload}, _from, state)
    ...
    channel =
      Map.put(
        ...
        channel_name,
        %Mycelium.Servers.Subserver.Structs{type: type, state: init_payload}
      )
  ...
  def handle_call({:create_channel, channel_name, type}, _from, state) do
    channel =
      Map.put(
        ...
        %Mycelium.Servers.Subserver.Structs{type: type, state: %{}}
      )
